...
std::string ActionParser::configurationToSSC32UCommandString(
    const franka_low_level_driver::FrankaConfigurationGoalConstPtr goal) const
{
  ...
  for(std::size_t i = 0; i < goalSize; ++i)
  {
    try
    {
      ...
      {
        command << "S" << touSPerSecond(goal->configuration[i].channel, (goal->configuration[i].speed * 100));
      }
    }
  ...
  if(goal->speedFactor != 0.0)
  {
    command << "T" << (goal->speedFactor * 100);
  }
...
void ActionParser::setTargetPositions(const franka_low_level_driver::FrankaConfigurationGoalConstPtr goal)
{
  unsigned long goalSize = goal->configuration.size();
...
unsigned long long
ActionParser::getLastFinishedTime(const franka_low_level_driver::FrankaConfigurationGoalConstPtr goal) const
{
  ...
  if(goal->speedFactor > 0.0)
  {
    if(lastFinishedTime > goal->speedFactor)
    {
      return lastFinishedTime;
    }
    return goal->speedFactor;
  }
  return lastFinishedTime;
}
