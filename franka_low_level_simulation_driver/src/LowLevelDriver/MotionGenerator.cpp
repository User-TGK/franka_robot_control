...
std::array<double, 7> MotionGenerator::limitRate(const std::array<double, 7>& max_derivatives,
  ...
  for(size_t i = 0; i < 7; i++)
  {
    ...
    else
    {
      ...
      else if(t >= t_2_sync_[i] && t < t_f_sync_[i])
      {
        (*delta_q_d)[i] =
            delta_q_[i] +
            0.5 * (1.0 / std::pow(delta_t_2_sync_[i], 3.0) * (t - t_1_sync_[i] - 2.0 * delta_t_2_sync_[i] - t_d[i]) *
...
void MotionGenerator::calculateSynchronizedValues()
{
  ...
  for(size_t i = 0; i < 7; i++)
  {
    if(std::abs(delta_q_[i]) > kDeltaQMotionFinished)
    {
      ...
      {
        dq_max_reach[i] = std::sqrt(4.0 / 3.0 * delta_q_[i] * sign_delta_q[i] * (ddq_max_start_[i] * ddq_max_goal_[i]) /
